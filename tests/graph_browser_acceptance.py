    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        try:
            ...
            with sync_playwright() as pw:
                ...
                limit_text=page.locator('#graph-anomaly-profiles').inner_text()
                check('profile_live_limit_rates',page.locator('#graph-anomaly-profiles .anomaly-profile-limit').count()==2 and '%/ч' in limit_text and 'Накоплено' not in limit_text)
                ...
                check('limit_rate_axis',page.locator('#graph-svg .graph-limit-axis').count()>0)
