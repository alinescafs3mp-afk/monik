    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with TestClient(app,base_url='http://127.0.0.1:8866') as client, sync_playwright() as pw:
            ...
            try:
                ...
                page.click('#pause');held=page.locator('#feed').inner_text();emit(200,'WHILE_DISPLAY_PAUSED');page.wait_for_timeout(650)
                check('pause_freezes_screen_not_collector',page.locator('#feed').inner_text()==held and len(app.state.store.events({'q':'WHILE_DISPLAY_PAUSED'})['items'])==1)
                page.click('#pause');page.get_by_text('WHILE_DISPLAY_PAUSED',exact=True).wait_for(timeout=4000);check('resume_catches_up',True)
                page.evaluate("location.hash='at=invalid'");page.wait_for_timeout(150);check('bad_history_link_keeps_ui',page.locator('#workspace').is_visible())
                def dense():
                    for i in range(50):emit(300+i,'DENSE_'+str(i));time.sleep(.06)
                producer=threading.Thread(target=dense);producer.start()
                page.get_by_text('DENSE_5',exact=True).wait_for(timeout=2200)
                check('continuous_notifications_do_not_starve_render',producer.is_alive())
                producer.join();page.wait_for_timeout(600)
                page.click('[data-tab="overview"]');page.wait_for_timeout(350)
                check('technical_settings_remain_verbatim','ultra' in page.locator('#content').inner_text() and 'gpt-6-astra' in page.locator('#content').inner_text())
                page.set_viewport_size({'width':1440,'height':1000});page.screenshot(path=str(out/'final-desktop.png'),full_page=True)
                page.set_viewport_size({'width':390,'height':844});page.screenshot(path=str(out/'final-mobile.png'),full_page=True)
