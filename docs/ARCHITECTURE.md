...
Rate limits: latest snapshot по profile/limit_id/window-role. Основное число `remaining=100-used`, только если 0 <= used <= 100. Изменения windows, reset time и немонотонность видимы; причина неизвестна. Счётчика «сумма падений процента» нет.
