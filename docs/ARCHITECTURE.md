...
Rate limits: owner-facing проекция выбирает последнее `codex/primary` по профилю. При наличии `logs_db` свежие значения берутся из уже сохранённых Codex response headers, привязанных к доказанному `thread_id`; это пассивное чтение, без `/status`, provider-запроса или app-server RPC. Дополнительные модельные квоты остаются сохранёнными событиями и не смешиваются с основным лимитом. Экран лимитов сохраняет `remaining=100-used`, только если 0 <= used <= 100. График накопленный процент не показывает: live-метрика вычисляет скорость изменения одного и того же provider-окна в `%/ч` по целевому интервалу 30 минут, адаптивно 60/120 минут при неизменившемся целочисленном счётчике. Токены в проценты не конвертируются. Общая чёрная линия берёт максимальный доступный темп профиля, не сумму; смена reset/window, немонотонность и нехватка истории дают неизвестное, а не ноль.
