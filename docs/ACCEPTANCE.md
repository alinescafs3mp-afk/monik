...
В v0.1.3 к закрытому производственному аудиту добавлены запрошенные владельцем изменения:
...
- устаревшее основное значение подписано как последнее известное, поскольку monik не опрашивает provider и не подменяет `/status`;
...
| Проверка | Результат |
|---|---:|
| Полный `unittest` | 161 PASS, 0 FAIL, 0 SKIP |
| Контрольные фикстуры | 11/11 PASS |
| Настоящий loopback HTTP/SSE | 23/23 PASS |
| Native Chromium: основной web | 47/47 PASS |
